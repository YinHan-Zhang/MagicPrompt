...
from wan.models import (AutoencoderKLWan, AutoencoderKLWan3_8, AutoTokenizer, CLIPModel,
                               WanT5EncoderModel, Wan2_2Transformer3DModel)
from wan.data.dataset_image_video import process_pose_file
from wan.models.cache_utils import get_teacache_coefficients
from wan.pipeline import Wan2_2FunControlPipeline
from wan.utils.fp8_optimization import (convert_model_weight_to_float8,
                                               convert_weight_dtype_wrapper,
                                               replace_parameters_by_name)
from wan.utils.lora_utils import merge_lora, unmerge_lora
from wan.utils.utils import (filter_kwargs, get_image_latent, get_image_to_video_latent,
                                    get_video_to_video_latent,
                                    save_videos_grid)
from wan.utils.fm_solvers import FlowDPMSolverMultistepScheduler
from wan.utils.fm_solvers_unipc import FlowUniPCMultistepScheduler

GPU_memory_mode     = None #"sequential_cpu_offload"
ulysses_degree      = 1
ring_degree         = 1
...
# model path
model_name          = "MagicPrompt/models/Wan2.2-Fun-5B-Control"
...
transformer_path        = "MagicPrompt/checkpoint-1000/transformer"
...
if transformer_path is not None:
    ...
    if os.path.isdir(transformer_path):
        ...
        index_file = os.path.join(transformer_path, "model.safetensors.index.json")
        if os.path.exists(index_file):
            with open(index_file, "r") as f:
                index = json.load(f)
            shard_files = set(index["weight_map"].values())
            print(f"Found {len(shard_files)} safetensors shards from index")
        else:
            shard_files = [os.path.basename(f) for f in glob.glob(os.path.join(transformer_path, "*.safetensors"))]
            print(f"Found {len(shard_files)} safetensors files")
        
        state_dict = {}
        for shard_name in sorted(shard_files):
            shard_path = os.path.join(transformer_path, shard_name)

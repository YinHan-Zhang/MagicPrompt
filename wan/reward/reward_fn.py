...
# All reward models.
__all__ = ["AestheticReward", "HPSReward", "PickScoreReward", "MPSReward", "LatentReward"]
    ...
    def __init__(
        ...
        if self.version == "v2":
            ...
            self.model = ImprovedAestheticPredictor(
                encoder_path="MagicPrompt/models/CLIP-ViT-H-14-laion2B-s32B-b79K", predictor_path=self.predictor_path
            )
            ...
            self.model, _ = convert_v2_5_from_siglip(encoder_model_name="MagicPrompt/models/siglip-so400m-patch14-384")
...
class PickScoreReward(BaseReward):
    ...
    def __init__(
        self,
        model_path="MagicPrompt/models/PickScore_v1",
        device="cpu",
        dtype=torch.float16,
        max_reward=1,
        loss_scale=1,
        ...
        self.processor = AutoProcessor.from_pretrained("MagicPrompt/models/CLIP-ViT-H-14-laion2B-s32B-b79K", torch_dtype=self.dtype)
...
class MPSReward(BaseReward):
    ...
    def __init__(
        ...
        processor_name_or_path = "MagicPrompt/models/CLIP-ViT-H-14-laion2B-s32B-b79K"
        # https://huggingface.co/laion/CLIP-ViT-H-14-laion2B-s32B-b79K/blob/main/preprocessor_config.json
        # TODO: [transforms.Resize(224), transforms.CenterCrop(224)] for any aspect ratio.
        self.transform = transforms.Compose([
            transforms.Resize((224, 224), interpolation=transforms.InterpolationMode.BICUBIC),
...
class LatentReward(torch.nn.Module):
  
    def __init__(
        self,
        metric: str = "cosine",
        weighting: str = "linear",  
        aggregate_seq: bool = False,
        eps: float = 1e-8,
    ...
    def _step_weight(self, t: int, T: int) -> float:
        if self.weighting == "linear":
            return (t + 1) / T
        elif self.weighting == "exponential":
            return math.exp(-3 * (1 - (t + 1) / T))
        elif self.weighting == "uniform":
            return 1.0 / T
        
    def _compute_diff(self, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    def compute_gt_loss(self, latent: torch.Tensor, gt: torch.Tensor):
        diff = self._compute_diff(latent, gt)
        
        running_mean = self.running_mean.to(diff.dtype)
        running_var = self.running_var.to(diff.dtype)
        
        z = (diff - running_mean) / (running_var.sqrt() + self.eps)
        norm_diff = torch.sigmoid(z)
        
        loss = norm_diff
        reward = 1.0 - norm_diff
        
        if self.aggregate_seq and loss.dim() == 2:
            loss = loss.mean(dim=1)
            reward = reward.mean(dim=1)
        
        return loss.mean().float(), reward.mean().float()

    def forward(self, latent: list, uncond_latent: list):
        T = len(latent)
        assert len(uncond_latent) == T
        
        weighted_diff = 0
        total_weight = 0
        for t in range(T):
            w = self._step_weight(t, T)
            step_diff = self._compute_diff(latent[t], uncond_latent[t])  # [bs, seq]
            weighted_diff = weighted_diff + w * step_diff
            total_weight += w
        
        diff = weighted_diff / total_weight
        
        with torch.no_grad():
            batch_mean = diff.mean()
            batch_var = diff.var()
        ...
        z = (diff - self.running_mean) / (self.running_var.sqrt() + self.eps)
        norm_diff = torch.sigmoid(z)
        
        reward = 1.0 - norm_diff
        loss = norm_diff
        
        if self.aggregate_seq and reward.dim() == 2:
            reward = reward.mean(dim=1)
            loss = loss.mean(dim=1)
...
if __name__ == "__main__":
    ...
    prompt_list = [
        "The video features a young woman with long brown hair and glasses. She is wearing a beige top with a lace collar. In the first frame, she is looking up with her eyes closed, her mouth slightly open as if she is about to speak or sing. In the second frame, she is still looking up, but her eyes are open and she appears to be in a state of concentration or deep thought. In the third frame, she is looking down, her eyes are closed again, and she seems to be in a state of relaxation or contemplation. The background is blurred, but it appears to be an indoor setting with a blue wall. The style of the video is a close-up shot with a shallow depth of field, focusing on the woman's face and expressions.",
    ]
    ...
    aesthetic_reward_v2_5 = AestheticReward(
        encoder_path="MagicPrompt/models/siglip-so400m-patch14-384", version="v2.5", device="cuda", dtype=torch.bfloat16
    )
    ...
    pick_score = PickScoreReward(device="cuda", model_path="MagicPrompt/models/PickScore_v1", dtype=torch.bfloat16)

...
# The following multi-architecture xfuser attention processors are only needed for
# sequence-parallel (USP) multi-GPU training. They are imported lazily so that a partial
# checkout of the `wan` package (where these modules are absent) still imports cleanly.
def _optional_xfuser(name):
    try:
        module = __import__(f"wan.dist.{name}", fromlist=["_"])
...
_optional_xfuser("cogvideox_xfuser")
_optional_xfuser("flux2_xfuser")
_optional_xfuser("flux_xfuser")
_optional_xfuser("hunyuanvideo_xfuser")
_optional_xfuser("qwen_xfuser")
_optional_xfuser("z_image_xfuser")

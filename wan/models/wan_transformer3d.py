...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, soft_k, soft_v, dtype=torch.bfloat16, t=0, visualize=True):
        ...
        num_soft_tokens = 0
        if soft_k is not None:
            num_soft_tokens = soft_k.size(1)
            bs = k.shape[0]
            soft_k_batch = soft_k.expand(bs, -1, -1, -1)
            k = torch.cat([soft_k_batch, k], dim=1)
        
        if soft_v is not None:
            bs = v.shape[0]
            soft_v_batch = soft_v.expand(bs, -1, -1, -1)  
            v = torch.cat([soft_v_batch, v], dim=1)      

        if num_soft_tokens > 0 and seq_lens is not None:
            k_lens = seq_lens + num_soft_tokens
        else:
            k_lens = seq_lens
...
class WanT2VCrossAttention(WanSelfAttention):
    ...
    def __init__(self,
        ...
        self.num_soft_tokens = 64
        head_dim = dim // num_heads
        
        init_std = 1.0 / np.sqrt(head_dim) * 2.0
        self.text_soft_k = nn.Parameter(
            torch.randn(1, self.num_soft_tokens, num_heads, head_dim) * init_std
        )
        self.text_soft_v = nn.Parameter(
            torch.randn(1, self.num_soft_tokens, num_heads, head_dim) * init_std
        )
    ...
    def forward(self, x, context, context_lens, dtype=torch.bfloat16, t=0):
        ...
        if num_soft_tokens > 0 and context_lens is not None:
            context_lens = context_lens + num_soft_tokens
        else:
        ...
        num_soft_tokens = self.text_soft_k.size(1)
        soft_k_batch = self.text_soft_k.expand(b, -1, -1, -1) 
        k = torch.cat([soft_k_batch, k], dim=1)      
        
        soft_v_batch = self.text_soft_v.expand(b, -1, -1, -1)  
        v = torch.cat([soft_v_batch, v], dim=1)
...
class WanI2VCrossAttention(WanSelfAttention):
    ...
    def __init__(self,
        ...
        self.num_soft_tokens = 64
        head_dim = dim // num_heads
        # ✅ 使用更大的初始化：让 soft tokens 一开始就有更大的影响力
        init_std = 1.0 / np.sqrt(head_dim) * 2.0
        self.text_soft_k = nn.Parameter(
            torch.randn(1, self.num_soft_tokens, num_heads, head_dim) * init_std
        )
        ...
        print("using image2video cross attention ...")
    ...
    def forward(self, x, context, context_lens, dtype=torch.bfloat16, t=0):
        ...
        num_soft_tokens = self.text_soft_k.size(1)
        soft_k_batch = self.text_soft_k.expand(b, -1, -1, -1)  # [B, num_soft_tokens, num_heads, head_dim]
        k = torch.cat([soft_k_batch, k], dim=1)       # [B, num_soft_tokens+S, num_heads, head_dim]
        
        soft_v_batch = self.text_soft_v.expand(b, -1, -1, -1)  # [B, num_soft_tokens, num_heads, head_dim]
        v = torch.cat([soft_v_batch, v], dim=1)       # [B, num_soft_tokens+S, num_heads, head_dim]

        # ✅ 关键修复：更新 k_lens 以包含 soft tokens
        # 否则 Flash Attention 的 varlen 函数会截断 soft tokens！
        if num_soft_tokens > 0 and context_lens is not None:
            context_lens = context_lens + num_soft_tokens
        else:
            context_lens = context_lens
...
class WanAttentionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        init_std = 1.0 / np.sqrt(head_dim) * 2.0
        self.soft_k = nn.Parameter(
            torch.randn(1, self.num_soft_tokens, num_heads, head_dim) * init_std
        )
        self.soft_v = nn.Parameter(
            torch.randn(1, self.num_soft_tokens, num_heads, head_dim) * init_std
        )
        
        self.soft_scale = nn.Parameter(torch.ones(1, 1, dim))
        self.soft_bias = nn.Parameter(torch.zeros(1, 1, dim))
    ...
    def forward(
        ...
        y = self.self_attn(temp_x, seq_lens, grid_sizes, freqs, 
                          self.soft_k, self.soft_v, dtype, t=t)
        
        y = y * self.soft_scale + self.soft_bias
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        # embeddings
        x = [self.patch_embedding(u.unsqueeze(0)) for u in x] # [1, 48, 13, 64, 40] bfchw -> [1, 1536, 13, 64/2, 40/2]
        # add control adapter
        ...
        x = [u.flatten(2).transpose(1, 2) for u in x] # [1, 1536, 13, 64/2, 40/2] -> [1, 13 * (64/2) * (40/2), 1536]
        ...
        if self.teacache is not None:
            ...
            for idx, block in enumerate(self.blocks):
                if torch.is_grad_enabled() and self.gradient_checkpointing:
                    ...
                    if idx == 29 and reward_flag and denoise_step % 4 == 0:
                        latent_rewards.append(x.to("cpu", non_blocking=True))

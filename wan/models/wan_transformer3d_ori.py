...
class WanTransformer3DModel_ori(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    r"""
            ...
            for idx, block in enumerate(self.blocks):
                if torch.is_grad_enabled() and self.gradient_checkpointing:
                    ...
                    if idx == 29 and reward_flag and denoise_step % 4 == 0:
                        # latent_rewards.append({
                        #     f"{idx}": x.to("cpu")
                        # })
                        latent_rewards.append(x.to("cpu", non_blocking=True))
                else:
                    # latent_rewards = None # inference
                    # arguments
                    kwargs = dict(
                        e=e0,
                        seq_lens=seq_lens,
                    ...
                    if idx == 29 and reward_flag and denoise_step % 4 == 0:
                        # Match the gradient-checkpointing branch above: store a plain tensor on
                        # CPU (not a dict). This keeps downstream consumers (e.g. LatentReward) able
                        # to handle both code paths uniformly.
                        latent_rewards.append(x.to("cpu", non_blocking=True))
...
class Wan2_2Transformer3DModel_ori(WanTransformer3DModel_ori):
    r"""
    Wan diffusion backbone supporting both text-to-video and image-to-video.
    """
